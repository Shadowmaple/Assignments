...
public:
    ...
    static NFA* Closure(NFA* entry) {
        ...
        // 构建 E 可达的，用 '#' 代替
        Node* newNodeForStart = new Node{itoStr(nodeNum++)};
        Node* newNodeForEnd = new Node{itoStr(nodeNum++)};
        // 构造边
        Edge* edge1 = new Edge{newNodeForStart, entry->start, '#'};
        Edge* edge2 = new Edge{entry->start, newNodeForEnd, '#'};
        v.push_back(*edge1);
        v.push_back(*edge2);

        entry->edges = v;
        entry->start = newNodeForStart;
        entry->end = newNodeForEnd;
        return entry;
    }
    ...
    // 获取 # 下所有可达点
    void GetReachableNodesByDefaultSymbol(set<Node*>& reachableNodes) {
        queue<Node*> q;
        set<Node*> curNodes;
        Node* node;
        ...
        while (!q.empty()) {
            ...
            for (auto edge : this->edges) {
                // 固定 # 可达
                if (edge.start->name == node->name && edge.symbol == '#') {
                    // 若该节点未加入集合，则加入集合和队列
                    if (reachableNodes.count(edge.end) == 0) {
                        reachableNodes.insert(edge.end);
                        q.push(edge.end);
                    }
                }
...
void ConvertToDFA(NFA* entry) {
    ...
    set<char> symbols;
    for (auto e : entry->edges) {
        if (e.symbol != '#') symbols.insert(e.symbol);
    }
    ...
    reachableNodes.insert(entry->start);
    entry->GetReachableNodesByDefaultSymbol(reachableNodes);
    curState->elements = reachableNodes;
